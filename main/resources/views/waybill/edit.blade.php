@extends('app')
@section('title', 'Waybill - Edit Waybill')
@section('content')
<style>
.card-header {
...
<div class="col-xl-8 col-lg-9 col-md-9 col-sm-12 col-12">
                        ...
                        <div class="card" style="margin-top: -3px;">
                            <div class="card-body">
                                ...
                                <label>Recipient</label>
                                    <div class="input-group" id="recipient_card">
                                          <select class="form-control select2" name="recipient" id="recipient">
                                            <option selected value="{{ $waybill->recipient_id }}">Choose...</option>
                                ...
                                <div class="" id="recipient_card2">
                                    <br>
                                    <b style="font-size: 20px;" id="recipient_name"></b>
                        ...
                        <div class="card-body">
                             <div class="form-row">
                                <div class="form-group col-md-6">
                                  ...
                                  <input type="text" name="date_of_shipment" id="date_of_shipment" class="form-control" value="{{ $waybill->date_of_shipment }}" readonly id="" placeholder="Date of shipment">
                                </div>
                                <div class="form-group col-md-6">
                                ...
                                <div class="form-group col-md-6">
                                  ...
                                  <input type="text" class="form-control" id="reference_number" value="{{ $waybill->reference_number }}" name="reference_number">
                                </div>
        ...
        <div style="display: contents;" id="packages">
            ...
            <div class="col-xl-12 col-lg-9 col-md-9 col-sm-9 col-12" id="package1">
                <div class="card">
                    <div class="row" style="padding: 20px; padding-bottom: 50px;">
                        <div class="col-sm-12">
                            <div class="card-header"><h5 style="display: inline">Package Details</h5> <div class="float-right" style="margin-right: -20px;"><button onclick="addItem(document.getElementById('totalitemsCount').value)" style="height: 29px;" class="btn btn-primary btn-sm" type="button">Add item</button></div></div>
                            <div class="card-body">

                            @php

                            $items = json_decode($waybill->packages_item_name);
                            $count = 1;
                            @endphp
                            ...
                            @foreach($items as $k => $item)
                              @php
                                $count++;
                                 ...
                                 $package_name = json_decode($waybill->packages_item_name, true);
                                 $package_unit_price = json_decode($waybill->packages_item_unit_price, true);
                                 $packages_qty = json_decode($waybill->packages_item_qty, true);
                                 $packages_total = json_decode($waybill->packages_item_total_price, true);

                                 // dd($package_name);

                              @endphp
                              <div id="item_card_{{ $count }}">
                                    <div class="form-row align-items-center" id="package{{ $count }}">
                                        <div class="col-sm-4">
                                          <label class="" for="inlineFormInput">Item name</label>
                                          <input type="text" class="form-control mb-2 packages_item_name" id="package{{ $count }}_item_name" placeholder="" value="{{ $package_name[$k] }}" name="packages_item_name[]">
                                        </div>

                                        <div class="col-sm-2">
                                          <label class="" for="inlineFormInput">Unit Price</label>
                                          <input type="number" class="form-control mb-2 unit-price packages_item_unit_price" onkeyup="total_price({{ $count }})" placeholder="0" value="{{ $package_unit_price[$k] }}" id="package{{ $count }}_item_unit_price" name="packages_item_unit_price[]">
                                        </div>

                                        <div class="col-sm-2">
                                          <label class="" for="inlineFormInput">Qty</label>
                                          <input type="number" class="form-control mb-2 qty packages_item_qty" onkeyup="total_price(1)" placeholder="" id="package{{ $count }}_item_qty" value="{{ $packages_qty[$k] }}" name="packages_item_qty[]">
                                        </div>

                                        <div class="col-sm-3">
                                          <label class="" for="inlineFormInput">Total Price</label>
                                          <input type="text" value="{{ $packages_total[$k] }}" readonly="" value="0.00" class="form-control mb-2 total-price packages_item_total_price" id="package{{ $count }}_item_total_price" name="packages_item_total_price[]">
                                        </div>
                                      @if($count <> 2)
                                        <div class="col-sm-1">
                                          <button onclick="deleteItem({{ $count }})" type="button" style="margin-top: 25px;
    margin-left: -4px;" class="btn btn-sm btn-outline-danger mb-2">Delete</button>
                            ...
                            <input id="totalitemsCount" name="totalitemsCount" value="{{ $count }}" hidden />
                            

                                    <div class="form-row align-items-center">
                                        <div class="col-sm-6">
                                          <label class="" for="inlineFormInput">Weight (Kg)</label>
                                          <input type="number" class="form-control mb-2" value="{{ $waybill->packages_weight }}" id="packages_weight" name="packages_weight" placeholder="">
                                        </div>
                                      </div>
                                </div>
                            </div>
        ...
        <div class="row">
        <button class="btn btn-primary" style="margin-left: 30px;" id="generate_waybill">Edit Waybill</button>
    </div>
...
<script>
    ...
    $('#generate_waybill').on('click', function(e) {
        ...
        // send request
        axios.post('/api/waybill/edit/{{ $waybill->waybill_code }}', {
            data
        })
        .then(function (response) {
            let data = response.data;
            ...
            if(data.code == 400) {
                ...
                Swal.fire({
                      title: '<strong>Waybill has been modified</strong>',
                      type: 'success',
                      html:
                        data.message,
                      showCloseButton: true,
