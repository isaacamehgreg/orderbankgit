...
/* controls */
	...
	loadBlockGroups : function() {
     ...
     loadServerGroups : function() {
        ...
        serversList.each(function ()
        {

            var list = $(this);
            var type = this.dataset.type;

            for (group in Vvveb.ServersGroup)
            {
                list.append('<li class="header" data-section="' + group + '"  data-search=""><label class="header" for="' + type + '_serverhead_' + group + '">' + group + '  <div class="header-arrow"></div>\
                                       </label><input class="header_check" type="checkbox" checked="true" id="' + type + '_serverhead_' + group + '">  <ol></ol></li>');

                var serversSubList = list.find('li[data-section="' + group + '"]  ol');
                servers = Vvveb.ServersGroup[ group ];

                for (i in servers)
                {
                    serverType = servers[i];
                    server = Vvveb.Servers.get(serverType);

                    if (server)
                    {
                        item = $('<li data-section="' + group + '" data-drag-type=server data-type="' + serverType + '" data-search="' + server.name.toLowerCase() + '"><a href="#">' + server.name + "</a></li>");

                        if (server.image) {

                            item.css({
                                backgroundImage: "url(" + ((server.image.indexOf('//') == -1) ? 'libs/builder/':'') + server.image + ")",
                                backgroundRepeat: "no-repeat"
                            })
                        }

                        serversSubList.append(item)
                    }
                }
            }
        });
...
/* iframe highlight */
	...
	_initBox: function() {
        ...
        $(".servers-list li ol li", addSectionBox).on("click", function(event) {
            var html = Vvveb.Servers.get(this.dataset.type).html;
	...
	_initDragdrop : function() {
		...
		$('.drag-elements-sidepane ul > li > ol > li').on("mousedown touchstart", function(event) {
			...
			if ($this.data("drag-type") == "component")
                ...
                self.component = Vvveb.Servers.get($this.data("type"));
...
Vvveb.Gui = {
	...
	saveAjax : function () {
		...
		return Vvveb.Builder.saveAjax(url, null, function (data) {
			$('#message-modal').modal().find(".modal-body").html("File saved to the database.");
		});
	},
    ...
    serverSearch : function () {
        ...
        $("#left-panel .servers-list li ol li").each(function () {
            $this = $(this);
    ...
    addBoxServerSearch : function () {
        searchText = this.value;

        $("#add-section-box .serves-list li ol li").each(function () {
            $this = $(this);
